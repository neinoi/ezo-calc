...
import ezocalc.operation
import ezocalc.value

OPERATORS = ['*','/','+','-','^']

class Mapper():
    _operation: ezocalc.operation.Operation

    def __init__(self):
        self._operation = ezocalc.operation.Operation()
    ...
    def _map_simple(self, sub):
        ...
        for l in sub:
            if l in OPERATORS:
                add_ope = True
                if len(acc) > 0:
                    splits.append(ezocalc.value.Value(acc))
                    acc = ""
                ...
                if add_ope:
                    splits.append(ezocalc.operand.Operand(l))
            else:
                acc += l
