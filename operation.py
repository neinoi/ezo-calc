...
class Operation(bloc.Bloc):
    ...
    def get_value(self) -> Number:
        ...
        while pos > 0:
            ...
            self._blocs[pos-1] = value.Value(val)
    ...
    def _find_ope(self):
        ...
        for e in self._blocs:
            if type(e) is operand.Operand and e.is_first():
                return (pos, e)
            pos += 1

        pos = 0
        for e in self._blocs:
            if type(e) is operand.Operand and e.is_second():
                return (pos, e)
            pos += 1

        pos = 0
        for e in self._blocs:
            if type(e) is operand.Operand:
                return (pos, e)
            pos += 1
